...
Dir <- '.../Step7.SubclusterDEG/sca.SubClusters.Objs.rds'
scaObjs <- readRDS(Dir)

for (i in 1:length(scaObjs)){
  data <- scaObjs[[i]]
  scaObjs[[names(scaObjs)[i]]] <- data
}              
...
MASTDEG <- function(data, job, instance) {# instance is inherited from the last step
  setwd(".../Step13.SubCluster.DEG/DEGData")
  Dir <- ".../Step13.SubCluster.DEG/DEGData"
  ...
  if (instance$strata == "comb"){
    if(instance$Ref == "E2"){
      ...
      data@colData$VaD <- relevel(as.factor(data@colData$VaD), ref = "No")
      ...
      dir.create(paste0(Dir, "/", Cluser), showWarnings = FALSE)
...
# Creating jobs
# We got a total of 53 subclusters
