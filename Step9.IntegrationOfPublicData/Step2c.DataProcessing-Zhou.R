...
## all folders
SampleLists <- list.files("RawDat")

## Define instance
# define instance (data preparation)
subsample <- function(data, job, i) {
  sampleID <- data[i]
  Dirs <- unlist(lapply(data, function(x){paste0(".../Step9.IntegrationOfPublicData/Zhou/RawDat/", x)}))
  dat <- Read10X(Dirs[i])
...
snRNAseqClean <- function(data, job, instance) { # instance is inherited from the last step
  ...
  SaveDir <- ".../Step9.IntegrationOfPublicData/Zhou/SeuratObj.Ind"
