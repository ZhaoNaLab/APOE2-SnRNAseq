...
reg <- makeExperimentRegistry("SnRNAseq.Registry",
                              packages = c("tidyverse", "Seurat", "data.table", "magrittr",
                                           "singleCellTK", "purrr", "DoubletFinder",
                                           "celda", "ggpubr", "biomaRt", "magrittr"), seed = 20221005)

reg <- loadRegistry("SnRNAseq.Registry", writeable = TRUE)

# load the combined SCE objects
## all folders
SampleLists <- list.files(pattern = "Don*")

## Define instance
# define instance (data preparation)
subsample <- function(data, job, i) {
  sampleID <- data[i]
  Dirs <- unlist(lapply(data, function(x){paste0(".../Step9.IntegrationOfPublicData/Sadick/", x)}))
  dat <- Read10X(Dirs[i])

  # Prepare column Metadata 
  ColumnMeta <- as.data.frame(colnames(dat))
  ColumnMeta$orig.ident <- data[i] 
  ColumnMeta %<>% column_to_rownames("colnames(dat)")
  
  # Prepare rownames data
  sce <- SingleCellExperiment(list(counts = dat), colData = ColumnMeta)
  sce <- decontX(sce)
...
snRNAseqClean <- function(data, job, instance) { # instance is inherited from the last step
  ...
  SaveDir <- ".../Step9.IntegrationOfPublicData/Sadick/SeuratObj.Ind"
